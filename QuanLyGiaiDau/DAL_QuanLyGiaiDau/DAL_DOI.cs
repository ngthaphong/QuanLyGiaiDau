...
namespace DAL_QuanLyGiaiDau
{
    public class DAL_DOI : DBConnect
    {
        ...
        //get cauthu use madoi
        public DataTable getCauThu(string mad)
        {
            string str = string.Format("SELECT TenDoi,HoTenCT,SoAo FROM DOI D, CAUTHU CT WHERE D.MaDoi=CT.MaDoi AND D.MaDoi='{0}'", mad);
            SqlDataAdapter data = new SqlDataAdapter(str, connect);
            DataTable dt = new DataTable();
            data.Fill(dt);
            return dt;
        ...
        public string getMa(string ten)
        {
            ...
            str = "";
            data.Fill(dt);
            if(dt.Rows.Count>0) str = dt.Rows[0].Field<string>(0);
            return str;
        }
        //Lay het user, pass
